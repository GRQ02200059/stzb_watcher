...
6. **Android 三图画廊**
   ...
   - 队伍与团队；
   - 战斗模拟；
   - 三张竖屏手机截图横排，使用 HTML table 保证 GitHub 展示稳定；
   - 每张图下方用一句话说明与 Web 端一致的业务能力。
...
- Web 使用当前本地服务 `http://127.0.0.1:8080`；
- Android 使用当前已连接模拟器或真机上的 `astzb` App；
- 使用当前页面真实数据；
- 不伪造统计值，不插入演示数据；
...
- 三张 Android 图使用同一设备、同一分辨率和竖屏方向；
- 优先使用 WebP，兼顾清晰度和仓库体积；
- 单张主图目标不超过 450KB；
- 单张画廊图目标不超过 300KB；
- 单张 Android 图目标不超过 250KB；
- 十张图片总目标不超过 3MB；
- 不增加 GIF、视频或持续动画资源。
...
- 打城考勤：任务阶段和成员区域可见；
- 玩家队伍：队伍列表和主要操作区可见。
- Android 战场总览：战场动态、筛选和主要状态可见；
- Android 队伍与团队：队伍或团队列表、主要统计和导航可见；
- Android 战斗模拟：双方阵容、配置入口和执行区域可见。
