...
def generate_team_report(player_teams):
    ...
    for player, teams in player_teams.items():
        if len(teams['attack_teams']) > 0 or len(teams['defend_teams']) > 0:
            # 攻击方队伍统计（只记录队伍配置，不记录使用次数）
            attack_team_details = []
            for team in sorted(teams['attack_teams']):
                attack_team_details.append({
                    '队伍配置': team
                })
            
            # 防守方队伍统计（只记录队伍配置，不记录使用次数）
            defend_team_details = []
            for team in sorted(teams['defend_teams']):
...
def save_team_reports(detailed_report):
    ...
    summary_df.to_csv('player_team_summary_unique.csv', index=False, encoding='utf-8-sig')
    print(f"队伍统计摘要已保存: player_team_summary_unique.csv ({len(summary_df)} 个玩家)")
    
    # 保存详细队伍配置
    with open('player_team_details_unique.json', 'w', encoding='utf-8') as f:
        json.dump(detailed_report, f, ensure_ascii=False, indent=2)
    print(f"详细队伍配置已保存: player_team_details_unique.json")
    ...
    team_details_df.to_csv('player_team_details_unique.csv', index=False, encoding='utf-8-sig')
    print(f"详细队伍配置CSV已保存: player_team_details_unique.csv ({len(team_details_df)} 条记录)")
    
    # 生成简要报告
    with open('team_analysis_report_unique.md', 'w', encoding='utf-8') as f:
        ...
        for player_data in detailed_report:
            for team_detail in player_data['攻击方队伍详情']:
                team_usage[team_detail['队伍配置']] += 1
            for team_detail in player_data['防守方队伍详情']:
                team_usage[team_detail['队伍配置']] += 1
        
        f.write(f"- 不同队伍配置数: {len(team_usage)} 种\n\n")
        
        f.write("## 🏆 最受欢迎队伍配置 (前10名)\n")
        f.write("（按使用该配置的玩家数量排序）\n")
        for i, (team, count) in enumerate(team_usage.most_common(10), 1):
            f.write(f"{i}. **{team}** - {count} 个玩家使用\n")
        ...
        f.write("\n## 📁 详细数据文件\n")
        f.write("- 队伍统计摘要: `player_team_summary_unique.csv`\n")
        f.write("- 详细队伍配置: `player_team_details_unique.csv`\n")
        f.write("- 完整数据(JSON): `player_team_details_unique.json`\n")
    
    print("简要报告已保存: team_analysis_report_unique.md")

def main():
    """主函数"""
    print("开始分析玩家队伍配置（真正去重版本）...")
