...
class Skill200938(Skill):
    def __init__(self):
        super().__init__(200938,'三术奇谋','1回合准备，对敌军单体3次谋略攻击(178%)，依次使攻防谋降低18持续2回合',2,50,False,4)
...
class Skill200952(Skill):
    def __init__(self):
        super().__init__(200952,'抚民励德','第2/4/6回合行动时我军全体谋略防御+80并受伤-20%持续2回合',1,'--',False,3)
    def call(self, hero, target=None):
        if hero.manager.round != 0: return
        hero.manager.log(hero, f'发动【{self.name}】')
        sk = self
...
class Skill200960(Skill):
    def __init__(self):
        super().__init__(200960,'鸾凤和鸣','每回合首次发动主动后我军群体2目标恢复85%；行动时我军全体3目标控制效果额外对1目标生效',1,'--',False,3)
    def call(self, hero, target=None):
        if hero.manager.round != 0: return
        hero.manager.log(hero, f'发动【{self.name}】')
        sk = self
...
class Skill200984(Skill):
    ...
    def call(self, hero, target=None):
        ...
        for t in hero.get_target(sk.limit, 2):
            ...
            t2_list.append(t)
        if t1 and len(set(id(x) for x in t2_list)) == 1 and t2_list and t2_list[0] is t1:
...
class Skill200986(Skill):
    def __init__(self):
        super().__init__(200986,'盛气横凌','普通攻击后对目标猛攻(260%)；士气高昂则混乱1回合；士气一般/低落则额外攻击(80%-160%)',3,50,False,0)
    def call(self, hero, target=None):
