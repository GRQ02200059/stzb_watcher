...
> Android 当前定位为 **独立抓包与核心分析 Beta**。核心移动闭环已经原生可用；复杂管理和研究能力按“经典兼容”或“Web 独有”明确标注，真实游戏抓包必须完成六阶段设备验收后才能视为正式 1.0。
...
| 团队 | 团数据、成员管理、打城考勤、自定义积分 | 支持 | 团数据、考勤、自定义积分原生 |
| 分析 | 排行、州郡分布、阵容研究与对阵证据 | 支持 | 排行与阵容研究原生；州郡经典兼容 |
| 模拟 | 双方配置、批量推演、结果与事件回放 | 支持 | 支持 |
| 系统 | 本地采集、SQLite、多档案、导出与设置 | 支持 | 抓包、SQLite、多档案、导出原生 |
...
Android 一级导航固定为 `战场 / 战报 / 同盟 / 工具`。队伍、实时部队、攻城考勤、自定义积分、阵容研究、团队报表、模拟器、地图、公告、排行、账号区服、抓包与经典兼容入口位于“工具”。

Android 状态定义：

- **原生**：Compose 页面和本机数据链路直接可用；
- **经典兼容**：仍由本机 `DashboardActivity` 承载，不依赖 Web 服务；
- **Web 独有**：当前只在 Web 提供；
- **规划中**：已进入 Android 1.0 迁移计划，不能当作当前可用能力宣传。

真实抓包闭环必须依次通过：`Native 组件 → VPN 通道 → SOCKS 连接 → 已知协议 → 本地入库 → 停止与网络恢复`。
...
8. 回到 `战场`、`战报`，或从 `工具` 打开 `队伍`、`地图与城池` 查看采集结果。
...
| 战报 | 查看完整战报并进入详情 |
| 同盟 | 查看同盟成员与分组 |
| 工具 → 队伍 | 按玩家、同盟、武将或战法搜索队伍 |
| 工具 → 团队报表 | 按周期、分组或成员查看团队报表，并导出 CSV |
| 工具 → 模拟器 | 配置攻守双方阵容，运行单场或批量战斗模拟 |
| 工具 → 地图与城池 | 查看地图格子、城池和类型统计 |
| 工具 → 排行榜 | 查看玩家武勋、同盟武勋和势力排行 |
| 工具 → 抓包启动台 | 选择目标 App、查看六阶段证据、启动/停止抓包和导出数据 |
