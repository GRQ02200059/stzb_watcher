...
fun CaptureConsoleScreen(
    ...
    LazyColumn(modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        item {
            Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(16.dp), Arrangement.spacedBy(10.dp)) {
                Text("目标与控制", style = MaterialTheme.typography.titleLarge)
                OutlinedTextField(
                    value = state.selectedApp?.let { "${it.label}\n${it.packageName}" }.orEmpty(), onValueChange = {}, readOnly = true,
                    label = { Text("目标 App") }, placeholder = { Text("尚未选择") }, modifier = Modifier.fillMaxWidth(),
                )
                OutlinedButton(onClick = { onIntent(CaptureConsoleIntent.LoadApps); appDialog = true }, Modifier.fillMaxWidth()) { Text("搜索并选择 App") }
                Row(Modifier.fillMaxWidth(), Arrangement.spacedBy(8.dp)) {
                    ...
                    OutlinedButton(
                        onClick = { onIntent(CaptureConsoleIntent.Stop) }, enabled = !state.busy,
                        modifier = Modifier.weight(1f).heightIn(min = 48.dp),
                    ) { Text("停止抓包") }
                }
            } }
        ...
        item {
            Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(16.dp), Arrangement.spacedBy(10.dp)) {
                Text("STZB 解析日志", style = MaterialTheme.typography.titleLarge)
                OutlinedTextField(
        ...
        item {
            Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(16.dp), Arrangement.spacedBy(8.dp)) {
                Text("导出与兼容", style = MaterialTheme.typography.titleLarge)
...
@Composable private fun StatusCard(state: CaptureConsoleUiState) {
    Card(Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(
        ...
        Text(if (state.running) "抓包运行中" else "抓包未启动", style = MaterialTheme.typography.titleLarge)
