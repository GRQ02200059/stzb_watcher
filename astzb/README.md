...
- App 默认启动页已切换为深色 Compose“实时战场”。
- 一级导航固定为：`战场 / 战报 / 同盟 / 更多`。
- “战场”已迁移为 Compose 实时事件 Feed，支持分类筛选、暂停/恢复和新事件缓冲提示。
- “战报”和“同盟”在批次 1 仍通过“打开经典页面”进入原 `DashboardActivity`，尚未迁移为完整 Compose 页面。
- “更多”提供“经典抓包控制台”和“经典数据页面”两个明确兼容入口。
...
8. 返回 Compose `战场` 页查看实时动态；经典数据仍可从 `更多 -> 经典数据页面` 进入
...
- 已验证：新 Launcher 默认进入“实时战场”、四个一级入口可达、“更多”兼容入口可见、经典抓包控制台可打开、Debug APK 含 arm64-v8a 与 armeabi-v7a 并可安装。
