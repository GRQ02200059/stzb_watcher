...
fun RankingsScreen(state: RankingsUiState, viewModel: RankingsViewModel, onBack: () -> Unit, modifier: Modifier = Modifier) {
    Column(modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Text("排行与团队报表", style = MaterialTheme.typography.headlineSmall, modifier = Modifier.padding(top = 10.dp))
        }
    ...
    LazyColumn(modifier, verticalArrangement = Arrangement.spacedBy(8.dp)) {
        items(rows, key = { "${state.category}:${it.rank}:${it.name}" }) { row ->
            Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(14.dp), Arrangement.spacedBy(4.dp)) {
                ...
                val metric = when (state.category) { RankingCategory.BATTLE -> "战功 ${row.value} · ${row.battles} 场 · 胜率 ${"%.1f".format(row.winRate)}%"; RankingCategory.UNION -> "势力 ${row.value} · ${row.members} 人"; RankingCategory.PLAYER_POWER -> "势力 ${row.value}" }
                Text(metric)
            } }
        }
    }
